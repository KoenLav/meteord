[![Circle CI](https://circleci.com/gh/koenlav/meteord/tree/master.svg?style=svg)](https://circleci.com/gh/koenlav/meteord/tree/master)
...
* `node-12-base`, `node-12.14.0-base`
* `node-12-binbuild`, `node-12.14.0-binbuild`
* `node-12-onbuild`, `node-12.14.0-onbuild`
* `node-12-devbuild`, `node-12.14.0-devbuild`
...
For brevity, not all possibilities are listed above and there are many more available.  It's recommended that you use the latest version within the series which your Meteor was designed for (see titles above).  The most recent version will be tagged with a `node-x-*` tag accordingly.  For the full list, please see the ["Tags" tab](https://hub.docker.com/r/koenlav/meteord/tags/) above.
...
For that, you can use `koenlav/meteord:onbuild` as your base image. Magically, that's only thing you have to do. Here's how to do it:

Add following `Dockerfile` into the root of your app:

~~~shell
FROM koenlav/meteord:onbuild
~~~
...
> WARNING: Don't use `koenlav/meteord:devbuild` for your final build. If you used it, your image will carry the Meteor distribution as well. As a result of that, you'll end up with an image with ~700 MB.
...
This sample docker-compose.yml file starts up a container that has used koenlav/meterod as its base and a mongo container. It also passes along several variables to Meteor needed to start up, specifies the port number the container will listen on, and waits 30 seconds for the mongodb container to start up before starting up the Meteor container.
